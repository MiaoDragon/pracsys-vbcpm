...
class Occlusion():
    ...
    def get_total_occlusions_v2(self, depth_img, color_img, camera):
        """
        ...
        cam_transform = np.linalg.inv(camera['extrinsics'])
        ...
        occluded = (cam_to_voxel_depth - voxel_depth > 0.) & valid_mask
        print(occluded.astype(int).sum() / valid_mask.astype(int).sum())
        return occluded

    def get_occlusions(self, depth_img, color_img, camera, obj_poses, obj_pcds):
        """
        ...
        """
        occluded = self.get_total_occlusions_v2(depth_img, color_img, camera)
